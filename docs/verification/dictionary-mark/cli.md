...
All CLI subsystem tests in `CliSubsystemTests.cs` pass; all requirements listed in the
...
**Scenario**: A YAML input file containing one entry is written to a temporary directory.
`Context.Create(["--input", inputFile, "--section", "CLI Depth", "--depth", "2", "--silent", "--log", logFile])` +
`Program.Run`.

**Expected**: `context.HeadingDepth` is 2, log contains "## CLI Depth", exit code 0.
